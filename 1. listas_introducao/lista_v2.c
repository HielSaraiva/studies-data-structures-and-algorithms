...
int inserir(struct lista *p, int num, int mode)
{
    ...
    // Se o mode = 0, o num ser� inserido no final. Qualquer valor diferente para mode far� num ser inserido no come�o
    // Se for um conjunto e ja houver o valor, retornar� -2
	if (p->tamanho != p->cap)
	{
		if (p->type == 0) {
			if(buscar(p, num) != -1) {
